...
do
	...
	# For pressure level variables, concatenate all monthly files first
	for var in u z;
	do
		# Find and concatenate all monthly files for this year and variable
		monthly_files=""
		for month in $(seq -w 1 12); do
			file="oifs/atm_regular_1m_pl_${var}_1m_pl_${i}${month}-${i}${month}.nc"
			if [ -f "$file" ]; then
				monthly_files="$monthly_files $file"
			fi
		done
		if [ ! -z "$monthly_files" ]; then
			if [ "$var" = "u" ]; then
				cdo sellevel,30000 -cat $monthly_files ${outdir}/${var}_$(printf "%04d" $i)_${tmpstr}_lvl.nc &
			elif [ "$var" = "z" ]; then
				cdo sellevel,50000 -cat $monthly_files ${outdir}/${var}_$(printf "%04d" $i)_${tmpstr}_lvl.nc &
			fi
		fi
	done
done
...
do
	...
	# For OIFS surface variables, concatenate all monthly files
	for var in ci 2t ttr tcc cp lsp 10u 10v;
	do
		# Find and concatenate all monthly files for this year and variable
		monthly_files=""
		for month in $(seq -w 1 12); do
			file="oifs/atm_regular_1m_${var}_1m_${i}${month}-${i}${month}.nc"
			if [ -f "$file" ]; then
				monthly_files="$monthly_files $file"
			fi
		done
		if [ ! -z "$monthly_files" ]; then
			cdo cat $monthly_files ${outdir}/${var}_${tmpstr}.nc &
		fi
	done
