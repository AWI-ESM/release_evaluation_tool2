...
def load_yearly_data_simple(path, var, years, pattern, freq):
    ...
    try:
        ...
        # For configs that span an OIFS output-config switch (e.g. AWI-ESM3-VEG-HR,
        # whose spinup straddles a CMIP7 output cutover at year 1650, with
        # different accumulation periods on either side), divide by a per-year
        # array instead of the single scalar. Configs without the split fall
        # back to the scalar `accumulation_period`.
        if var != '2t':
